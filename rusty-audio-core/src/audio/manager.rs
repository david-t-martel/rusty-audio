...
use super::backend::{
    AudioBackend, AudioBackendError, AudioConfig, AudioStream, DeviceInfo, Result, StreamDirection,
    StreamStatus,
};
...
impl AudioDeviceManager {
    ...
    pub fn create_output_stream(&self, config: AudioConfig) -> Result<()> {
        let device = self.selected_output_device.read().clone().ok_or_else(|| {
            AudioBackendError::DeviceNotFound("No output device selected".to_string())
        })?;
    ...
    pub fn create_input_stream(&self, config: AudioConfig) -> Result<()> {
        let device = self.selected_input_device.read().clone().ok_or_else(|| {
            AudioBackendError::DeviceNotFound("No input device selected".to_string())
        })?;
    ...
    pub fn play(&self) -> Result<()> {
        ...
        if let Some(stream) = stream_lock.as_mut() {
            ...
            Err(AudioBackendError::StreamError(
    ...
    pub fn get_device_configs(&self, device_id: &str) -> Result<Vec<AudioConfig>> {
        let backend = self.backend.read();
        backend.supported_configs(device_id, crate::audio::backend::StreamDirection::Output)
    }
