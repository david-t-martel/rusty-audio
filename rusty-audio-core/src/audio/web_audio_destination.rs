...
/// Thread-safe wrapper for AudioContext in WASM destination
///
/// WASM is single-threaded, but we need Send/Sync for trait compatibility.
/// This wrapper uses Rc<RefCell<>> internally and provides Send/Sync impls
/// that are safe because WASM execution is single-threaded.
#[cfg(target_arch = "wasm32")]
struct WasmAudioContext {
    context: Rc<RefCell<AudioContext>>,
}
...
// The AudioContext is never actually sent between threads in WASM
#[cfg(target_arch = "wasm32")]
unsafe impl Send for WasmAudioContext {}
#[cfg(target_arch = "wasm32")]
unsafe impl Sync for WasmAudioContext {}
...
impl WebAudioDestination {
    ...
    pub fn new(
        ...
        Self {
            context: WasmAudioContext::new(context),
            sample_rate,
            channels,
    ...
    pub fn flush_to_output(&mut self) -> Result<()> {
        ...
        // Create source node
        let source = ctx.create_buffer_source().map_err(|e| {
            super::backend::AudioBackendError::Other(anyhow::anyhow!(
