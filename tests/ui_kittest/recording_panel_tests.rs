...
use rusty_audio::audio::recorder::{MonitoringMode, RecordingState};
use rusty_audio::ui::{
...
fn test_recording_state_transitions_ui() {
    ...
    assert_eq!(
        panel.recorder().expect("Recorder should exist").state(),
        RecordingState::Idle
    );
...
fn test_buffer_clear() {
    ...
    let initial_pos = recorder
        .buffer()
        .lock()
        .expect("Should lock buffer")
        .position();
