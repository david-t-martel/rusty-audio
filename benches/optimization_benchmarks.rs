...
use rusty_audio::audio_performance::{
    OptimizedSpectrumProcessor, 
    OptimizedEqProcessor,
    OptimizedBufferPool,
};
    ...
    for pool_size in &[8, 16, 32] {
        for buffer_size in &[512, 1024, 2048, 4096] {
            ...
            group.bench_with_input(
                BenchmarkId::new("v1_acquire_release", format!("{}x{}", pool_size, buffer_size)),
                buffer_size,
                |b, &size| {
                    b.iter(|| {
            ...
            group.bench_with_input(
                BenchmarkId::new("v2_acquire_release", format!("{}x{}", pool_size, buffer_size)),
                buffer_size,
                |b, &size| {
...
fn benchmark_memory_bandwidth(c: &mut Criterion) {
    ...
    for size in &[1024, 4096, 16384, 65536, 262144] {
        ...
        group.bench_with_input(
            BenchmarkId::new("unsafe_memcpy", size),
            size,
            |b, &size| {
                b.iter(|| {
                    unsafe {
...
fn benchmark_end_to_end(c: &mut Criterion) {
    ...
    let mut optimized_pipeline = ZeroCopyAudioPipeline::new(BLOCK_SIZE, NUM_BANDS, SAMPLE_RATE, FFT_SIZE);
    ...
    group.bench_function("optimized_complete", |b| {
        b.iter(|| {
            for _ in 0..NUM_ITERATIONS {
                let spectrum = optimized_pipeline.process_zero_copy(&input, &mut output, &mut analyser);
                black_box(&output);
                black_box(spectrum);
            }
        });
