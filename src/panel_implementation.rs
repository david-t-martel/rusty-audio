...
impl PanelContent for AudioPlayerApp {
    fn show_file_browser(&mut self, ui: &mut egui::Ui) {
        ...
        ui.vertical(|ui| {
            ...
            if let Some(file) = &self.current_file {
                ...
                ui.label(RichText::new(file.file_name()).color(colors.accent).strong());
            } else {
    ...
    fn show_waveform(&mut self, ui: &mut egui::Ui) {
        ...
        ui.vertical(|ui| {
            ...
            ui.label(RichText::new(format!("{} / {}", current_time, total_time)).color(colors.text));
        });
    }
                ...
                egui::ComboBox::from_label("")
                    ...
                    .show_ui(ui, |ui| {
                        for mode in &[SpectrumMode::Bars, SpectrumMode::Line] {
                            if ui.selectable_value(&mut self.spectrum_visualizer.config_mut().mode, mode.clone(), format!("{:?}", mode)).clicked() {
                                // Mode changed
                            }
                        }
                    });
    ...
    fn show_effects(&mut self, ui: &mut egui::Ui) {
        ...
        ui.vertical(|ui| {
            ...
            ui.horizontal(|ui| {
                ui.label("L");
                let panning_slider = egui::Slider::new(&mut self.panning, 0.0..=1.0)
                    .show_value(false);
            ...
            ui.label(RichText::new("Future effects will be added here:").color(colors.text_secondary));
            ui.label("• Reverb");
            ui.label("• Delay");
    ...
    fn show_inspector(&mut self, ui: &mut egui::Ui) {
        ...
        ui.vertical(|ui| {
            ...
            ui.group(|ui| {
                ...
                ui.separator();
                ui.label(format!("Sample Rate: {} Hz", self.audio_context.sample_rate()));
                ui.label(format!("Playback State: {:?}", self.playback_state));
                ui.label(format!("Volume: {:.1}%", self.volume * 100.0));
                ui.label(format!("Loop: {}", if self.is_looping { "On" } else { "Off" }));
            });
    ...
    fn show_transport(&mut self, ui: &mut egui::Ui) {
        ...
        ui.vertical_centered(|ui| {
            ...
            ui.horizontal_centered(|ui| {
                ...
                if ui.add_sized(button_size, egui::Button::new("📁 Open")).clicked() {
                    self.open_file_dialog();
                }
                ...
                if ui.add_sized(button_size, egui::Button::new("⏹️ Stop")).clicked() {
